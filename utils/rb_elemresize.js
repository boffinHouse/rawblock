...
const elementResize = {
    ...
    addMarkup: rb.rAF(function (element, data) {
        let fire, widthChange, heightChange;
        let first = true;
        let expandElem, shrinkElem, expandChild, block;
        const posStyle = 'position:absolute;top:0;left:0;display: block;'; //
        const wrapperStyle = posStyle + 'bottom:0;right:0;';
        ...
        const onScroll = rb.throttle(function () {
            if (block) {
                return;
            }
            ...
            const curWidthChange = width != data.width;
            const curHeightChange = height != data.height;
...
export default $.fn.elementResize;
