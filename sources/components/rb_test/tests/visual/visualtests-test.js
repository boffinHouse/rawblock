//var casperHelpers = require('../../../../../tests/visual/casper-helpers');
//
//casper.thenOpen(casperHelpers.root + 'test-test_page.html')
//    .then(function() {
//        var promise = casperHelpers.deferred();
//
//        casper.waitFor(casperHelpers.waitForJsReady(), function(){
//
//            phantomcss.screenshot('#test-1', 'test-default');
//
//            casper.wait(9, function () {
//                casper.click('#test-1 .test-btn-next');
//                casper.wait(66, function () {
//                    phantomcss.screenshot('#test-1', 'test-clicked');
//                    promise._resolve();
//                });
//            });
//
//        });
//
//        return promise;
//    })
//;
