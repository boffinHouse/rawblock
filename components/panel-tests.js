(function (global, factory) {
    ...
    (function () {
        ...
        var events = {
            ...
            panelchanged: function panelchanged() {
                events.changedIndex++;
            }
        };
        ...
        QUnit.testStart(function (obj) {
            ...
            rb.$('#qunit-fixture').html('<button type="button" aria-controls="panel-id-id-1" class="rb-button js-rb-click" data-module="panelbutton">Toggle</button>' + '<div id="panel-id-id-1" class="rb-panel js-panel" data-module="panel">' + '<input class="js-rb-autofocus" />' + '<button type="button" class="panel-close">sdsd</button>' + '</div>');

            Object.keys(events).forEach(function (evt) {
                var type = _typeof(events[evt]);
                if (type == 'function') {
        ...
        QUnit.test('panel and panelbutton2', function (assert) {
            ...
            QUnit.afterAF(30).then(function () {
                ...
                rb.$(panel).rbComponent().open();
                syn.click(document.querySelector('#qunit-fixture'));
                return QUnit.afterAF(99).then(function () {
                    assert.strictEqual(rb.$(panel).rbComponent().isOpen, false);
                });
            }).then(function () {
                rb.$(panel).rbComponent().open();
                ...
                syn.click(panel.querySelector('.panel-close'));
