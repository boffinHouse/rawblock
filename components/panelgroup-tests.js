(function (global, factory) {
    if (typeof define === "function" && define.amd) {
        ...
        factory();
        global.panelgroupTests = mod.exports;
    }
})(this, function () {
    'use strict';
    ...
    (function () {
        ...
        QUnit.testStart(function (obj) {
            ...
            rb.$('#qunit-fixture').html('<div class="rb-panelgroup" data-module="panelgroup">' + '<button type="button" class="panelgroup-btn">1</button>' + '<button type="button" class="panelgroup-btn">2</button>' + '<button type="button" class="panelgroup-ctrl-btn" data-type="prev">&lt;</button>' + '<button type="button" class="panelgroup-ctrl-btn" data-type="next">&gt;</button>' + '<div class="panelgroup-panel">' + '{{panelContent}}' + '</div>' + '<div class="panelgroup-panel">' + '{{panelContent}}' + '</div>' + '</div>');
        });
        ...
        QUnit.test('rb-panelgroup double change', function (assert) {
            ...
            QUnit.afterAF(90).then(function () {
                assert.deepEqual(panelComponent.selectedIndexes, [0]);
                assert.ok($panelGroup.find('.panelgroup-panel').get(0).classList.contains('is-open'));
                assert.notOk($panelGroup.find('.panelgroup-panel').get(1).classList.contains('is-open'));
            }).then(done);
