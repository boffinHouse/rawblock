...
Object.assign(Position.prototype, {
    ...
    setOptions: function (options) {
        const {my, at, collision} = this.options;
    ...
    addOffset: function (posType, box, offset, useMargin) {
        let marginIndex;
        const props = Position.posType[posType];

        let ret = (box[posType] + box[props.dim] * (offset[props.axisIndex] / 100));
    ...
    calculatePosition: function (posType, targetBox, elementBox, targetOffset, elementOffset, computedPosition) {
        let rootDim, rootPos, isOut1, isOut2, targetOffset2, elementOffset2;

        const props = Position.posType[posType];

        let position = this.addOffset(posType, targetBox, targetOffset) - this.addOffset(posType, elementBox, elementOffset, true) + this.pos[posType];
